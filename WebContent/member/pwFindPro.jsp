...
<% 	}else{	
		...
		if(auto != null && id != null && pw != null){	//cookie가 존재해서 그 아이디가 존재한다면 로그인 시켜주기
			...
			<%	
				...
				member = dao.findPw(id, cellnum);			//비밀번호 찾아주는메서드 실행
			
				if(member != null){ //객체에 내용물이 담겼을 경우 즉, 해당 아이디에 대한 비밀번호가 존재할 경우
			%>		
					<%=id%>님의 비밀번호는  <%=member.getPw() %>입니다
					<button onclick="window.location='login.jsp'">확인</button>
			<%	}else{//객체가 비었을떄 즉, 찾는 정보가 없을경우 %>		
					<script>
						alert("입력하신 정보는 없는 정보입니다.");
						history.go(-1);
					</script>
			<%	} %>
			</div>
